...
// Modified from HAPI FHIR 8.10.0: explicit profile resolution is mandatory,
// independently of the policy for in-band profile assertions.
class ValidatorWrapper {
	...
	public List<ValidationMessage> validate(
		...
		List<ValidationMessage> invalidExplicitProfileMessages = new ArrayList<>();
		for (String nextProfileUrl : theValidationContext.getOptions().getProfiles()) {
			fetchAndAddExplicitProfile(theWorkerContext, profiles, nextProfileUrl, invalidExplicitProfileMessages);
		}
		...
		// Explicit requests remain errors after the in-band policy has been applied.
		messages.addAll(invalidExplicitProfileMessages);
		return messages;
	}
	...
	private void fetchAndAddExplicitProfile(
		...
		// Resolver execution failures propagate unchanged; only absence is invalid input.
		...
		if (structureDefinition != null) {
			...
			m.setMessage("Invalid profile. Failed to retrieve explicitly requested profile with url=" + theUrl);
